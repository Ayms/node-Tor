...
####The usual trivial question is how can you be sure that the js OP code loaded by the browser from our site is trustable? It is if you use https with our site combined with the excellent [Interception Detector] (http://www.ianonym.com/intercept.html)
...
A file is represented by a link (https://www.download.com/file), which can be an internet link or generated by the system if this is a file that you have stored yourself inside your browser.

This link is processed to get a unique reference called a "Hash" below.

Files are stored inside your browser using IndexedDB. Files are stored using blocks so if an unexpected event does occur or someone closed his browser during the download, the transfer can be resumed from another source.
