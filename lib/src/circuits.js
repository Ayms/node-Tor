...
const {simpleParser,choose_id,ValToIP,IPtoVal}=require('./utils.js');
const {client_tls_options,init_socket,init_socket_}=require('./sockets2.js');
const tls=require('tls');
const fs=require('fs');
const {request_start,init_connected_request,on_data,clear_circuits_OR_out,init_request,destroy_ws_cid}=require('./requests.js');
...
Circuit.prototype = {
	...
	versions_cell_handle:function(cell) {
		...
		if ((!this.OR_)||(this.onion_)) { //OP or OR handshake
			...
			if (!socket_.ws_) {
				...
				cert1.Certificate=new Buffer(createIdLinkTLSCert('./'+OR_name+'/pub-key.pem','./'+OR_name+'/priv-id-key.pem','der',socket_.certid_,socket_.cert_date,socket_.cert_subject,socket_.cert_issuer),'hex');
				...
				cert2.Certificate=new Buffer(createIdLinkTLSCert('./'+OR_name+'/pub-id-key.pem','./'+OR_name+'/priv-id-key.pem','der',parseInt((Rand(8)).toString('hex'),16),socket_.cert_date,socket_.cert_issuer,socket_.cert_issuer),'hex');
		...
		if (request) {
			...
			if (request.params_.stream) {
				let cd=request.params_.stream;
					while (cd.length) {
						if (!tid) {
							let stream=new Stream(Stream.prototype.RELAY_DATA,id,cd.slice(0,Math.min(cd.length,Stream.prototype.PAYLOAD_STREAM)),this.Df_hash);
						} else {
							let stream=new Stream(Stream.prototype.RELAY_DB_DATA,id,[new Buffer(tid,'hex'),cd.slice(0,Math.min(cd.length,Stream.prototype.PAYLOAD_STREAM-16))].concatBuffers(),this.Df_hash);
						};
						let cell=new Cell(this.circId,Cell.prototype.RELAY,this.stream_encrypt_forward(stream));
						request.sent_++;
	...
	change_or: function(msg,cb) {
		...
		let i,n_or;
		if (this.first_) {
			let request=this.first_.request_;
		};
		...
		if ((!this.prev_&&one_OR)&&(typeof(this.server_)==='undefined'?false:(this.server_===one_OR))) {
			...
			let n_or=this.prev_?(this.next_?Relays:Exit):Guards;
			let l=n_or.length;
			this.clear_timers();
			this.clear_t0_();
			if (this.path_) {
	...
	clear_t0_: function() {
		...
		if (id) {
			...
			let request=c[id]?c[id]:c.request_;
		};
		msg=msg.split(':');
		...
		if (tab.length===3) { //specific
			...
			if (data.indexOf('WebSocket')!=-1) { //websocket
				...
				//handle tls over WS
					...
					let certid=parseInt((Rand(8)).toString('hex'),16);
					let cert=createIdLinkTLSCert(pathd+OR_name+'/pub-key.pem',pathd+OR_name+'/priv-key.pem','pem',certid,date,servername,issuer);
					sslcontext=tls.createSecureContext({key: fs.readFileSync(pathd+OR_name+'/priv-key.pem'),cert: cert,servername: servername});
					//let cleartext=new tls.TLSSocket(request,{secureContext:sslcontext}); //TODO ?? see https://github.com/nodejs/node/issues/29559
					let pair=new tls.createSecurePair(sslcontext,true);
					request.pair=pair;
					...
					cleartext.on('data',function(data) {
						//console.log('cleartext received: '+data.slice(0,20).toString('hex'));
