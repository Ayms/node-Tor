...
*/
  ...
  cipher.update2 = function(input) {
	if (input) {
		if(input.length()) {
		  // not finishing, so fill the input buffer with more input
		  _input.data=_input.data.substr(_input.read);
		  _input.read=0;
		  _input.putBuffer(input);
		}
	};
	//console.log('cipher')
	//console.log(_input.toHex()+' '+_input.length()+' '+_input.read);
	//console.log(_inBlock);
    // do cipher operation while input contains full blocks or if finishing
    while(_input.length() >= _blockSize) {
      _op();
    }
	if (cipher.overflow) {
		_output.getBytes(cipher.overflow);
	};
	//console.log(_output.toHex()+' '+cipher.overflow);
	//alert(_outBlock);
	var overflow = _input.length() % _blockSize;
	if (overflow) {
		var prev=forge.util.createBuffer(_input.data.slice(_input.read));
		//console.log(prev.toHex());
		//console.log('cipher');
		var prev_in=_inBlock.slice(0);
...
forge.aes.createCipheriv=function(algo,key,iv) {
	var mode=algo.split('-')[2];
	var output=forge.util.createBuffer();
	key=forge.util.createBuffer(key.toString('binary'));
	iv=forge.util.createBuffer(iv.toString('binary'));
	var cipher=forge.aes.startEncrypting(key,iv,output,mode);
	var oupdate=cipher.update2;
	cipher.update=function(data) {
		var res;
		if (data) {
			data=forge.util.createBuffer(data.toString('binary'));
		} else {
